...
int vglCLAllocPbo(VglImage* img){
  ...
  if (dbg) printf("vglCLAllocPbo: started, clPbo = %d.\n", img->clPbo);
  if (img->cudaPbo == (unsigned int)-1){
    ...
    glBufferData(GL_PIXEL_PACK_BUFFER, img->width * img->height * img->nChannels, 0, GL_STREAM_DRAW);
    ERRCHECK()
  }
  else{
    glBindBuffer(GL_PIXEL_PACK_BUFFER, img->cudaPbo);
...
int vglGlToCuda(VglImage* img){
  ...
  ERRCHECK()
  glReadPixels(0, 0, img->width, img->height, GL_RGB, GL_UNSIGNED_BYTE, (GLvoid*) 0);
  ERRCHECK()
...
int vglCudaToGl(VglImage* img){
  ...
  ERRCHECK()
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img->width, img->height, 0, GL_RGB, GL_UNSIGNED_BYTE, (GLvoid*) 0);
  ERRCHECK()
